...
public final class SyncProtocol {
    ...
    public record State(String partyId, byte myLevel, boolean isPublic, byte publicJoinLevel, boolean paused, int currentIndex, boolean autoRemovePlayed, List<Track> tracks, List<Member> members) {}
    ...
    public static State readState(DataInputStream d) throws IOException {
        ...
        for (int i = 0; i < count; i++) tracks.add(new Track(d.readUTF(), d.readUTF()));
        ...
        return new State(partyId, myLevel, isPublic, publicJoinLevel, paused, index, autoRemovePlayed, tracks, members);
    }

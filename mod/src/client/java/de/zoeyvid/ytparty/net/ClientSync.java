...
public final class ClientSync {
    ...
    public static void register() {
        PlayerController.INSTANCE.setBackend(SERVER_SINK);

        ClientPlayNetworking.registerGlobalReceiver(SyncPayload.TYPE,
            (payload, context) -> { if (RelayClient.INSTANCE.connected()) return; context.client().execute(() -> dispatch(payload.data())); });
    ...
    public static void dispatch(byte[] data) {
        ...
        try (DataInputStream d = SyncProtocol.reader(data)) {
            ...
            switch (op) {
                ...
                case Opcodes.S2C_MESSAGE -> message(d.readUTF());
                case Opcodes.S2C_PUBLIC_LIST -> {
                    int count = d.readInt();

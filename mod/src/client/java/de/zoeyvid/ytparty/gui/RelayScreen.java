...
public final class RelayScreen extends Screen {
    ...
    protected void init() {
        ...
        hostField.setTooltip(Tooltip.create(Component.literal("The relay server address")));
        ...
        portField.setTooltip(Tooltip.create(Component.literal("The relay port (default 25599)")));
        ...
        addRenderableWidget(Button.builder(Component.literal("Remember password: " + (RelayClient.rememberPassword ? "ON" : "OFF")), b -> {
            ...
            if (!RelayClient.rememberPassword) RelayClient.autoConnect = false;
            de.zoeyvid.ytparty.ClientConfig.save();
            rebuildWidgets();
        }).tooltip(Tooltip.create(Component.literal("Keep the password saved between sessions"))).bounds(left, top + 78, 158, 20).build());
        Button auto = Button.builder(Component.literal("Autoconnect: " + (RelayClient.autoConnect ? "ON" : "OFF")), b -> {
            RelayClient.autoConnect = !RelayClient.autoConnect;
            de.zoeyvid.ytparty.ClientConfig.save();
            rebuildWidgets();
        }).tooltip(Tooltip.create(Component.literal("Reconnect to this relay automatically (needs a saved password)"))).bounds(left + 162, top + 78, 158, 20).build();
        auto.active = RelayClient.rememberPassword;
        ...
        if (busy) {
            addRenderableWidget(Button.builder(Component.literal("Disconnect"), b -> RelayClient.INSTANCE.disconnect()).tooltip(Tooltip.create(Component.literal("Disconnect from the relay"))).bounds(left, top + 104, 320, 20).build());
        } else {
    ...
    @Override
    public void removed() { de.zoeyvid.ytparty.ClientConfig.save(); }
